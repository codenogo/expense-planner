...
type BudgetQuery struct {
	...
	loadTotal     []func(context.Context, []*Budget) error
...
func (_q *BudgetQuery) QueryHousehold() *HouseholdQuery {
...
func (_q *BudgetQuery) WithHousehold(opts ...func(*HouseholdQuery)) *BudgetQuery {
	query := (&HouseholdClient{config: _q.config}).Query()
...
func (_q *BudgetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Budget, error) {
	...
	if query := _q.withHousehold; query != nil {
		if err := _q.loadHousehold(ctx, query, nodes, nil,
			func(n *Budget, e *Household) { n.Edges.Household = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *BudgetQuery) loadHousehold(ctx context.Context, query *HouseholdQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *Household)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Budget)
	for i := range nodes {
		if nodes[i].household_budgets == nil {
			continue
		}
		fk := *nodes[i].household_budgets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "household_budgets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *BudgetQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *Category)) error {
	...
	nodeids := make(map[int][]*Budget)
	for i := range nodes {
		if nodes[i].category_budgets == nil {
			continue
		}
		fk := *nodes[i].category_budgets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_budgets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
