...
//		GroupBy(transactionentry.FieldAmountCents).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.TransactionEntry.Query().
//		Select(transactionentry.FieldAmountCents).
//		Scan(ctx, &v)
...
func (_q *TransactionEntryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TransactionEntry, error) {
	...
	if _q.withTransaction != nil || _q.withAccount != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *TransactionEntryQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*TransactionEntry, init func(*TransactionEntry), assign func(*TransactionEntry, *Transaction)) error {
	...
	for i := range nodes {
		if nodes[i].transaction_entries == nil {
			continue
		}
		fk := *nodes[i].transaction_entries
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_entries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionEntryQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*TransactionEntry, init func(*TransactionEntry), assign func(*TransactionEntry, *Account)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_entries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
