...
func (_q *TagQuery) QueryHousehold() *HouseholdQuery {
...
func (_q *TagQuery) QueryTransactions() *TransactionQuery {
...
func (_q *TagQuery) WithHousehold(opts ...func(*HouseholdQuery)) *TagQuery {
	query := (&HouseholdClient{config: _q.config}).Query()
...
func (_q *TagQuery) WithTransactions(opts ...func(*TransactionQuery)) *TagQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withHousehold; query != nil {
		if err := _q.loadHousehold(ctx, query, nodes, nil,
			func(n *Tag, e *Household) { n.Edges.Household = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTransactions; query != nil {
		if err := _q.loadTransactions(ctx, query, nodes,
			func(n *Tag) { n.Edges.Transactions = []*Transaction{} },
			func(n *Tag, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedTransactions {
		if err := _q.loadTransactions(ctx, query, nodes,
			func(n *Tag) { n.appendNamedTransactions(name) },
			func(n *Tag, e *Transaction) { n.appendNamedTransactions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TagQuery) loadHousehold(ctx context.Context, query *HouseholdQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Household)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Tag)
	for i := range nodes {
		if nodes[i].household_tags == nil {
			continue
		}
		fk := *nodes[i].household_tags
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "household_tags" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TagQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Transaction)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.TransactionsTable)
		s.Join(joinT).On(s.C(transaction.FieldID), joinT.C(tag.TransactionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.TransactionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.TransactionsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "transactions" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *TagQuery) WithNamedTransactions(name string, opts ...func(*TransactionQuery)) *TagQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
